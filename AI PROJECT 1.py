...
print()
print()
print("\t\t\t WELCOME TO AKANKSHA'S LIBRARY\t\t\t")
print()
print()
...
class student:
    ...
    def displaystud(self):
        ...
        if(self.token==1):
            printf("\t Book Number is :",self.stbno)
    def modifystud(self):
        print()
        print("\t Admission Number of the student is :",self.admno)
        print()
...
def writestudent():
    ch="Y"
    fp=open("student1.dat","ab" )#pointer for file #3 types of modes 1.rb 2.wb 3.ab
    while ch=="Y":
        st.createstud()
        ch=input("\t\t Do you want to continue??<y/n>:")
        dump(st,fp) #dumping data from student class into student file.
        ch=ch.upper()
        print()
...
def modify_student_record():
    found=0 #found is used to check if student exists or not
    print()
...
def del_book():
    ...
    print()
    print()
    n=input("\t\t Enter Book Number to be deleted:")
    print()
    fin=open("book1.dat","rb")
    fout=open("temp.dat","wb")
    try:
...
def book_issue():
    ...
    try:
        ...
        try:
            while True:
                st=load(fin2) #object for student.dat
                if (st.ret_admno()==sn):
                    st.showstud()
...
def adminmenu():
    ch='Y'#first time to enter the loop directly
    while ch=="Y":
        print()
        print()
        ...
        if ch1==1:
            ...
            print()
            print()
            bn=input("\t\t ENTER BOOK NUMBER TO BE DIPLAYED:")
            display_spb(bn)
        elif ch1==9:
            modify_bookrecord()
        elif ch1==10:
            del_book()
            print()
            ch=input("\t\t Do you want to continue with ADMINMENU<y/n>:")
            ch=ch.upper()
            print()
            #os.system("cls")
            if ch=="Y":
